...
### Какие источники данных выбрала
...
**Итерация 1:** Бот работал, но при нажатии ➕ из старого поиска (после перезапуска) — падал, т.к. кэш `user_data` был пуст. Добавила fallback: если фильма нет в кэше, запрашиваем детали через API `/movie/{id}`.

**Итерация 2:** Описания фильмов обрезались на середине слова. Добавила обрезку до 150 символов с добавлением `…`.

**Итерация 3:** При оценке фильма хотелось видеть визуальное подтверждение. Добавила звёздочки: `⭐⭐⭐⭐` для оценки 8/10.
...
| Проблема | Решение |
|---|---|
| TMDB API возвращает `null` в `overview` для некоторых фильмов | Добавила `(item.get("overview") or "")` вместо `item["overview"]` |
